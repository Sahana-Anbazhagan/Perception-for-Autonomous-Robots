...
def update_p(p, Delta_p):
    ...
    p[0] += 1.5 * (Delta_p_0 + p[0] * Delta_p_0 + p[2] * Delta_p_1)
    p[1] += 1.5 * (Delta_p_1 + p[1] * Delta_p_0 + p[3] * Delta_p_1)
    p[2] += 1.5 * (Delta_p_2 + p[0] * Delta_p_2 + p[2] * Delta_p_3)
    p[3] += 1.5 * (Delta_p_3 + p[1] * Delta_p_2 + p[3] * Delta_p_3)
    p[4] += 1.5 * (Delta_p_4 + p[0] * Delta_p_4 + p[2] * Delta_p_5)
    p[5] += 1.5 * (Delta_p_5 + p[1] * Delta_p_4 + p[3] * Delta_p_5)
...
def get_coordinates(rect, w, h, warp_mat):
...
def main():
    ...
    for img in glob.glob((r"DragonBaby/DragonBaby/img/*.jpg")):
        image1.append(img)
    image1.sort()
    image1 = image1[1:]

    grey = cv2.imread('DragonBaby/DragonBaby/img/0001.jpg')
    ROI, template = get_template(grey, frame_increase)
    for img in image1:
        image = cv2.imread(img)
        grey = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

        grey = np.float32(grey) / 255.
        # Using Lk to track
        ...
        pts = get_coordinates(rect, w, h, warp_mat)
    ...
    height, width, k = image_list[0].shape
    size = (width, height)

    out = cv2.VideoWriter('baby_tracking.avi', cv2.VideoWriter_fourcc(*'DIVX'), 5.0, size)
